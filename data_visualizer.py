...
def _pick_best_data(all_data, user_request=""):
    ...
    Priority:
    1. If user says "direct" / "indirect" → match that view
    2. If a "Combined" view exists → use it (merged data from auto mode)
    3. If multiple views → merge them by first column
    4. Single dataset → use it
    """
    if not all_data:
        return None
    if len(all_data) == 1:
    ...
    # Check for explicit view request
    for d in all_data:
        view = (d.get("source") or "").lower()
        if "direct" in req_lower and "indirect" not in req_lower and "direct" in view and "indirect" not in view:
            return d
        if "indirect" in req_lower and "indirect" in view:
            return d

    # Prefer Combined view
    for d in all_data:
        view = (d.get("source") or "").lower()
        if "combined" in view:
            return d

    # No Combined — merge multiple views by first column (same logic as pipeline)
    # Check if all have the same first column
    first_cols = [d["columns"][0].upper() for d in all_data if d.get("columns")]
    if len(set(first_cols)) == 1 and first_cols[0]:
        dim_name = all_data[0]["columns"][0]
        # Find the primary numeric column name
        primary_col = all_data[0]["columns"][1] if len(all_data[0]["columns"]) > 1 else "VALUE"
        merged = {}
        for d in all_data:
...
class CodeEditorNode(Node):
    ...
    def build_output(self) -> Message:
        ...
        # Detect if user explicitly requested a chart type
