...
def _build_business_rules(rules_data):
    ...
    # Keep structured metric metadata when present so downstream components
    # can use formula/filter/usage directly instead of flattened strings.
...
def _index_examples(examples_data):
    ...
    # Deduplicate by id (knowledge files may contain duplicate example IDs)
    seen_ids = set()
    deduped = []
    for ex in examples:
        eid = ex.get("id", "")
...
class CodeEditorNode(Node):
    ...
    inputs = [
        ...
        HandleInput(
            ...
            info="From Knowledge Base component (primary / direct spend KB).",
            required=False,
        ),
        HandleInput(
            name="knowledge_base_indirect",
            display_name="Knowledge Base (Indirect)",
            input_types=["Data", "Message"],
            info="From a second Knowledge Base component for indirect spend. If connected, output is tagged by view.",
            required=False,
        ),
    ...
    def _process_single_kb(self, kb, debug_key="kb"):
        """Process a single Knowledge Base input into files list and debug info."""
        all_files = []
        debug_info = {}

        if not kb or kb == "":
            return all_files, debug_info

        debug_info[f"{debug_key}_type"] = str(type(kb).__name__)
        debug_info[f"{debug_key}_has_text"] = hasattr(kb, "text")
        debug_info[f"{debug_key}_has_data"] = hasattr(kb, "data")
        if hasattr(kb, "text"):
            txt = str(kb.text) if kb.text else ""
            debug_info[f"{debug_key}_text_length"] = len(txt)
            debug_info[f"{debug_key}_text_preview"] = txt[:300]
        if hasattr(kb, "data"):
            d = kb.data
            debug_info[f"{debug_key}_data_type"] = str(type(d).__name__)
            if isinstance(d, dict):
                debug_info[f"{debug_key}_data_keys"] = list(d.keys())[:10]
            elif isinstance(d, str):
                debug_info[f"{debug_key}_data_preview"] = d[:200]

        extracted = self._extract_files_from_kb(kb)
        all_files.extend(extracted)
        debug_info[f"{debug_key}_files_extracted"] = len(extracted)
        debug_info[f"{debug_key}_file_names"] = [f[0] for f in extracted]

        return all_files, debug_info

    def _build_knowledge_context(self, all_files, additional_rules="", additional_context=""):
        """Build a unified knowledge context dict from a list of (filename, parsed_content) tuples."""
        _NAME_MAP = {
    ...
    def build_output(self) -> Data:
        ...
        # ── Extract files from primary KB (direct) ──
        direct_files, direct_debug = self._process_single_kb(self.knowledge_base, "kb_direct")
        debug_info.update(direct_debug)

        # ── Extract files from indirect KB (if connected) ──
        kb_indirect = self.knowledge_base_indirect
        has_indirect = kb_indirect and kb_indirect != ""
        indirect_files = []
        if has_indirect:
            indirect_files, indirect_debug = self._process_single_kb(kb_indirect, "kb_indirect")
            debug_info.update(indirect_debug)
        ...
        if text_in and isinstance(text_in, str) and text_in.strip():
            ...
            if parsed is not None:
                direct_files.append(("text_input", parsed))

        if not direct_files and not indirect_files:
            self.status = "No knowledge inputs parsed"
            return Data(data={"error": True, "message": "No knowledge inputs parsed.", "_debug": debug_info})

        # ── Build knowledge contexts ──
        if has_indirect and indirect_files:
            # Dual-view mode: tag output by view
            direct_ctx = self._build_knowledge_context(direct_files, additional_rules, additional_context)
            indirect_ctx = self._build_knowledge_context(indirect_files, additional_rules, additional_context)

            direct_loaded = direct_ctx.get("knowledge_files_loaded", 0)
            indirect_loaded = indirect_ctx.get("knowledge_files_loaded", 0)
            ...
            self.status = (
                f"Direct: {direct_loaded}/14 ({direct_ctx.get('synonym_count',0)} syn) | "
                f"Indirect: {indirect_loaded}/14 ({indirect_ctx.get('synonym_count',0)} syn)"
            )

            parts = [
                f"**Knowledge Processor** (dual-view mode)\n",
                f"**Direct KB:** {direct_loaded}/14 files",
            ]
            for fd in direct_ctx.get("files_detected", []):
                parts.append(f"  - {fd['name']} ({fd['items']} items) [{fd['filename']}]")
            parts.append(f"\n**Indirect KB:** {indirect_loaded}/14 files")
            for fd in indirect_ctx.get("files_detected", []):
                parts.append(f"  - {fd['name']} ({fd['items']} items) [{fd['filename']}]")
        else:
            # Single-view mode (backward compatible)
            context = self._build_knowledge_context(direct_files, additional_rules, additional_context)
            context["_debug"] = debug_info

            loaded = context.get("knowledge_files_loaded", 0)
            self.status = f"Loaded: {loaded}/14 | {context.get('synonym_count',0)} synonyms, {context.get('entity_count',0)} entities, {context.get('example_count',0)} examples"

            parts = [f"**Knowledge Processor** (pure code -- no LLM)\n", f"**Files Detected:** {loaded}/14"]
            for fd in context.get("files_detected", []):
                parts.append(f"  - {fd['name']} ({fd['items']} items) [{fd['filename']}]")
            parts.append("\n**Knowledge Summary:**")
            for label, key in [("Synonyms", "synonym_count"), ("Entities", "entity_count"),
                                 ("Hierarchies", "hierarchy_count"), ("Examples", "example_count"),
                                 ("Columns", "column_count"), ("Entity Aliases", "entity_alias_count"),
                                 ("Anti-Patterns", "anti_pattern_count"), ("SQL Templates", "sql_template_count"),
                                 ("Column Values", "column_values_count")]:
